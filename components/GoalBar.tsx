...
import { GoalItem } from '../config/ownerConfig';

const SingleGoalBar: React.FC<{ goal: GoalItem }> = ({ goal }) => {
  ...
  return (
    <div ref={ref} className="mb-8">
      {isComplete && (
        <div className="p-3 rounded-xl text-center font-orbitron font-bold text-xs tracking-widest mb-4"
      ...
      <div className="flex justify-between items-end mb-2">
        <div>
          <p className="font-orbitron font-black text-white text-sm tracking-widest uppercase">{goal.title}</p>
      ...
      <div className="relative h-3 rounded-full overflow-hidden mb-3"
        ...
        <div className="h-full rounded-full transition-all duration-[1200ms] ease-out"
          style={{
            ...
            background: isComplete ? 'linear-gradient(90deg,#22c55e,#4ade80)' : 'linear-gradient(90deg,#00d4ff,#a855f7)',
            boxShadow: isComplete ? '0 0 16px rgba(34,197,94,0.5)' : '0 0 16px rgba(0,212,255,0.5)',
          }} />
        <div className="absolute inset-0 rounded-full"
          style={{ background: 'linear-gradient(90deg,transparent 40%,rgba(255,255,255,0.15) 50%,transparent 60%)', backgroundSize: '200% 100%', animation: 'shimmer 2s infinite' }} />
      </div>
      <div className="flex gap-2 flex-wrap">
        {goal.milestones.map((m, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all"
              style={{
...
const GoalBar: React.FC = () => {
  ...
  return (
    <div className="mb-12">
      {enabledGoals.map(goal => <SingleGoalBar key={goal.id} goal={goal} />)}
